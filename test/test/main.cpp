...
void on_mouse(int EVENT, int x, int y, int flags, void* userdata)
{
        ...
        case cv::EVENT_LBUTTONDOWN:
    {
        ...
        circle(hh, points, 4, cv::Scalar(255, 255, 255), -1);
...
int selectPolygon(cv::Mat srcMat, cv::Mat &dstMat)
{
    ...
    drawContours(m, contours, 0, cv::Scalar(0), -1);
...
int mouseROI()
{
    cv::Mat srcMat = cv::imread("/Users/xuyishen/Desktop/sa jiganyouhaximeiyouga/2020.5.8/1/img1.jpg");
    cv::Mat dstMat;
...
int dftDemo() {
    ...
    cv::Mat srcMat = cv::imread("/Users/xuyishen/Desktop/sa jiganyouhaximeiyouga/2020.5.8/1/img1.jpg", 0);
    ...
    //获得输入图像的最佳变换尺寸
    int m = cv::getOptimalDFTSize(srcMat.rows);
    int n = cv::getOptimalDFTSize(srcMat.cols);
    //对新尺寸的图片进行边缘边缘填充
    copyMakeBorder(srcMat, padMat, 0, m - srcMat.rows, 0, n - srcMat.cols, cv::BORDER_CONSTANT, cv::Scalar::all(0));

    //定义一个数组,存储频域转换成float类型的对象，再存储一个和它一样大小空间的对象来存储复数部分
    cv::Mat planes[] = { cv::Mat_<float>(padMat), cv::Mat::zeros(padMat.size(), CV_32F) };
    cv::Mat complexMat;
    ...
    */
    ...
    cv::Mat q1(magMat, cv::Rect(cx, 0, cx, cy));
    cv::Mat q2(magMat, cv::Rect(0, cy, cx, cy));
    cv::Mat q3(magMat, cv::Rect(cx, cy, cx, cy));
    cv::Mat tmp;
    q0.copyTo(tmp);
    q3.copyTo(q0);
    tmp.copyTo(q3);
...
{
    cv::Mat dst;
    cv::Mat src = cv::imread("../testImages\\rose.jpg", 0);

    int m = cv::getOptimalDFTSize(src.rows); //2,3,5的倍数有更高效率的傅里叶变换
    ...
    cv::Mat ph = cv::Mat_<float>(padded);
    cv::Mat complexImg;
